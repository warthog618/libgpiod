...
void print_chip_help(void)
{
	...
	printf("\t\t\tThe chip may be identified by number, name, or path,\n");
...
{
	printf("\nPeriods:\n");
	printf("  Periods are taken as microseconds unless a unit is specified, e.g. 10ms.\n");
...
static bool resolve_line(struct gpiod_line_info *info, struct line_resolver *resolver) {
	int i, found_idx = -1;
	unsigned int offset;
	...
	for (i = 0; i < resolver->num_lines; i++) {
		...
		if ((!line->id_is_name && (line->offset == (int)offset)) ||
			...
			found_idx = i;
			line->found = true;
			line->chip_idx = resolver->num_chips;
			line->offset = offset;
			resolver->num_found++;
		}
	}
				    ...
				    bool strict, bool by_name)
{
	...
	for (i = 0; (i < num_chips) && !resolve_done(resolver); i++) {
		...
		for (offset = 0; (offset < num_lines) && !resolve_done(resolver); offset++) {
			...
			if (resolve_line(line_info, resolver))
				chip_used = true;
			   ...
			   unsigned int offset)
{
	...
	for (i = 0; i < resolver->num_lines; i++)
		if ((resolver->lines[i].offset == (int)offset) &&
		    (resolver->lines[i].chip_idx == chip_idx) &&
...
void set_line_values(struct line_resolver *resolver, int chip_idx, int *values)
{
	...
	for (i = 0, j = 0; i < resolver->num_lines; i++) {
		if (resolver->lines[i].chip_idx == chip_idx) {
