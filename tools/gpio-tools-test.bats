...
regex_matches() {
	[[ $2 =~ $1 ]] || (echo "Mismatched: \"$2\"" && return 1)
}
...
output_is() {
	test "$output" = "$1" || (echo "Mismatched: \"$output\"" && return 1)
}

num_lines_is() {
	test ${#lines[@]} -eq $1 || (echo "Num lines is : ${#lines[@]}" && return 1)
}
...
dut_regex_match() {
	...
	read -t 0.2 -u ${COPROC[0]} LINE || (echo Timeout && return 1)
	if [ -n "$DUT_FIRST_CHAR" ]
	then
		LINE=${DUT_FIRST_CHAR}${LINE}
		unset DUT_FIRST_CHAR
	fi
	[[ $LINE =~ $PATTERN ]] || (echo "Mismatched: \"$LINE\"" && return 1)
}
